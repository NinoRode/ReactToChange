...
get_level <- function(pntz, prev_level) {
  ...
  while (np > 0) {
    ...
    num_max <- rowSums(t(vapply(1:np, function(i) {(maxs %in% pntz[i, ])}, numeric(dimz))))
    # num_max <- rowSums(t(vapply(1:np, function(i) {(maxs[cmpr] %in% pntz[i, cmpr])}, numeric(nc))))
    ...
    # } else {
    # }
    for_levl <- pntz[which(num_max > 0), , drop = FALSE]
...
build_tower <- function(pntz, one_level) {
  ...
  while (np >= dimz) {
    ...
    if (length(pntz_over) > 0)  { ############### stvar gre do konca ###############
      
      # p_hull_pntz <- rbind(p_hull_pntz, pntz_over) ######## stvar gre do konca ###
      
    # } else {
      
      pntz <- pntz_over
      one_level <- get_level(pntz, one_level)
