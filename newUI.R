...
server=function(input,output, session){
  ...
  display_dimensions <- reactive((as.character(c(input$dimension[1], input$dimension[2]))))
  dw <- 0
  output$dimension_display <- renderText(display_dimensions()[1])
  ...
  za_teden <- reactive(build_teden(default_df, as.integer(display_dimensions()[1])))

  col_widths = c(100, 60, 60, 60, 120)
